...
## NLP as Temporal Problem
  ...
  - Collobert: Tagging a word considers the whole sentence
    - Produces local features around each word of the sentence, combine into a global feature vector fed to standard affine layers
...
## Simple Convolution/Max-Over-Time CMOT Architecture
  ...
  - Here is a simple CNN that works very well on short texts
    - Convolutional layer generating between 60 and 500 feature maps
  ...
  - Use word2vec embeddings as pretraining for a Lookup Table
    - A lookup table is support multiply of 1-hot vector against shared weight matrix, yielding a single column
...
## Characters and Morphology
  ...
  - Characters are important for morphology
  ...
  - Would we get an improvement just by averaging? (obviously wouldnt likely be as good, but quick and dirty)
    - Seems to be a yes, even training using word2vec for embeddings for characters
